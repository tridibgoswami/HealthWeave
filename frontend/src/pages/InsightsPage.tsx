...
import {
  ...
  Heart, Zap, ShieldCheck, Info, BarChart3, Link2, AlertOctagon,
  CheckCircle2, Stethoscope, Network,
} from "lucide-react";
...
const SIG_CONFIG: Record<string, { bg: string; border: string; badge: string; dot: string; label: string }> = {
...
const DRUG_SEV_CONFIG: Record<string, { bg: string; border: string; badge: string; icon: string }> = {
  contraindicated: { bg: "bg-red-50",    border: "border-red-300",    badge: "bg-red-100 text-red-800",        icon: "text-red-600" },
  major:           { bg: "bg-orange-50", border: "border-orange-200", badge: "bg-orange-100 text-orange-800",  icon: "text-orange-600" },
  moderate:        { bg: "bg-amber-50",  border: "border-amber-200",  badge: "bg-amber-100 text-amber-800",    icon: "text-amber-600" },
  minor:           { bg: "bg-slate-50",  border: "border-slate-200",  badge: "bg-slate-100 text-slate-600",    icon: "text-slate-500" },
};
...
const TYPE_CONFIG: Record<string, { icon: React.ElementType; label: string; color: string }> = {
  ...
  disease_progression:            { icon: Activity,     label: "Disease Pattern",    color: "text-amber-600 bg-amber-100" },
  medicine_biomarker_correlation: { icon: Pill,         label: "Medicine Effect",    color: "text-purple-600 bg-purple-100" },
};
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2 mb-1">
              ...
              <span className={cn("text-[10px] font-bold px-1.5 py-0.5 rounded-full shrink-0", sig.badge)}>
                {sig.label}
              </span>
            </div>
            <p className="text-xs text-slate-500 leading-relaxed line-clamp-2">
              {finding.description || finding.ai_explanation}
            </p>
            {finding.entities?.length > 0 && (
              <div className="flex flex-wrap gap-1 mt-1.5">
                {finding.entities.slice(0, 4).map((e: string) => (
                  <span key={e} className="text-[10px] bg-white/70 border border-slate-200 text-slate-600 px-1.5 py-0.5 rounded-full font-medium">
                    {e.replace(/_/g, " ")}
                  </span>
                ))}
              </div>
      ...
      {open && (
        <div className="px-4 pb-4 space-y-3 border-t border-slate-200/80 pt-3">
          ...
          <p className="text-[10px] text-slate-400 flex items-start gap-1">
            ...
            Risk indicator only — not a medical diagnosis. Always consult a licensed healthcare professional.
          </p>
        </div>
      )}
    </div>
...
function DrugInteractionCard({ interaction, onAcknowledge }: {
  interaction: DrugInteraction;
  onAcknowledge: (id: string) => void;
}) {
  const [open, setOpen] = useState(false);
  const cfg = DRUG_SEV_CONFIG[interaction.severity] || DRUG_SEV_CONFIG.minor;
      ...
      <button className="w-full text-left p-4" onClick={() => setOpen(v => !v)}>
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-start gap-3 flex-1 min-w-0">
            <AlertOctagon size={16} className={cn("shrink-0 mt-0.5", cfg.icon)} />
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                <p className="text-sm font-bold text-slate-800">
                  {interaction.medicine_a?.replace(/_/g, " ")} + {interaction.medicine_b?.replace(/_/g, " ")}
                </p>
              </div>
              <span className={cn("text-[10px] font-bold px-1.5 py-0.5 rounded-full capitalize", cfg.badge)}>
                {interaction.severity}
              </span>
              {!open && (
                <p className="text-xs text-slate-500 mt-1 line-clamp-1">{interaction.description}</p>
              )}
            </div>
          </div>
      ...
      {open && (
        <div className="px-4 pb-4 space-y-3 border-t border-slate-200/80 pt-3">
          <div className="bg-white/80 rounded-xl p-3 border border-slate-100">
            <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wide mb-1">What Can Happen</p>
            <p className="text-xs text-slate-700 leading-relaxed">{interaction.description}</p>
          </div>
          {interaction.clinical_significance && (
            <div className="bg-white/80 rounded-xl p-3 border border-slate-100">
              <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wide mb-1">Clinical Significance</p>
              <p className="text-xs text-slate-700 leading-relaxed">{interaction.clinical_significance}</p>
            </div>
          )}
          {interaction.recommendation && (
            <div className="bg-white/80 border border-emerald-100 rounded-xl p-3">
              <div className="flex items-center gap-1.5 mb-1">
                <Stethoscope size={11} className="text-emerald-600" />
                <p className="text-[11px] font-bold text-emerald-700 uppercase tracking-wide">Recommendation</p>
              </div>
              <p className="text-xs text-slate-700 leading-relaxed">{interaction.recommendation}</p>
            </div>
          )}
          <button
            onClick={() => onAcknowledge(interaction.id)}
            className="w-full flex items-center justify-center gap-1.5 py-2 px-4 text-xs font-bold text-slate-600 bg-white border border-slate-200 rounded-xl hover:border-slate-300 transition-all"
          >
            <CheckCircle2 size={12} /> Acknowledged — will discuss with doctor
          </button>
        </div>
      )}
    </div>
...
export function InsightsPage() {
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "correlations" | "progression" | "medicine" | "interactions">("overview");
  ...
  const { data: interactionsData, isLoading: intLoading } = useQuery({
    queryKey: ["medicine-interactions"],
    queryFn: () => intelligenceApi.getMedicineInteractions(),
    staleTime: 5 * 60_000,
  });
  ...
  const intMut = useMutation({
    mutationFn: () => intelligenceApi.checkMedicineInteractions(),
    onSuccess: () => {
      toast.success("Drug interaction check started");
      setTimeout(() => qc.invalidateQueries({ queryKey: ["medicine-interactions"] }), 4000);
    },
  });

  const ackMut = useMutation({
    mutationFn: (id: string) => intelligenceApi.acknowledgeInteraction(id),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["medicine-interactions"] }); toast.success("Acknowledged"); },
  });

  const latestScore = (scoresData?.data?.scores || [])[0];
  const findings: CorrelationFinding[] = correlationsData?.data || [];
  const alerts = alertsData?.data || [];
  const interactions: DrugInteraction[] = interactionsData?.data?.interactions || [];
  ...
  const severeInteractions = interactions.filter(i => i.severity === "contraindicated" || i.severity === "major");
  ...
  const TABS = [
    ...
    { key: "interactions",  label: "Drug Interactions", count: interactions.length, alert: severeInteractions.length > 0 },
  ] as const;
  ...
  return (
    <div className="p-8 max-w-3xl">
      ...
      <div className="mb-7">
        ...
        <p className="text-sm text-slate-400 ml-[52px]">Longitudinal patterns, risk indicators, drug interactions and disease intelligence</p>
      </div>

      {/* Drug interaction urgent banner */}
      {severeInteractions.length > 0 && (
        <div className="flex items-start gap-3 bg-red-50 border border-red-300 rounded-2xl p-4 mb-5">
          <AlertOctagon size={18} className="text-red-600 shrink-0 mt-0.5" />
          <div>
            <p className="text-sm font-bold text-red-800 mb-0.5">
              {severeInteractions.length} serious drug interaction{severeInteractions.length > 1 ? "s" : ""} detected
            </p>
            <p className="text-xs text-red-600">Review the Drug Interactions tab and discuss with your doctor immediately.</p>
          </div>
          <button onClick={() => setActiveTab("interactions")} className="ml-auto text-xs font-bold text-red-700 underline shrink-0">
            View →
          </button>
        </div>
      )}
      ...
      <div className="flex gap-1 bg-slate-100 rounded-xl p-1 mb-6 overflow-x-auto">
        {TABS.map(tab => (
          <button
            ...
            className={cn(
              "flex-shrink-0 flex items-center justify-center gap-1.5 py-2 px-2.5 rounded-lg text-xs font-bold transition-all",
              activeTab === tab.key ? "bg-white text-slate-800 shadow-card" : "text-slate-500 hover:text-slate-700"
            )}
          >
            {tab.label}
            {tab.count !== null && tab.count > 0 && (
              <span className={cn(
                ...
                (tab as any).alert ? "bg-red-500 text-white animate-pulse" :
        ...
        <div className="space-y-5">
          <div className="bg-white rounded-3xl border border-slate-100 shadow-card p-6">
            <div className="flex items-center justify-between mb-5">
              <div>
                ...
                <div className="flex items-baseline gap-2">
                  <p className="text-4xl font-black text-slate-900">{scoresLoading ? "—" : (latestScore?.overall_score ?? "—")}</p>
              ...
              <button onClick={() => computeMut.mutate()} disabled={computeMut.isPending}
                ...
                <Zap size={11} />{computeMut.isPending ? "Computing…" : "Recompute"}
              </button>
            </div>
            ...
            {!latestScore && !scoresLoading && (
              <p className="text-xs text-slate-400 text-center mt-4">
                Upload health records then click Recompute to see your scores
              </p>
            )}
          </div>
            ...
            <div className="bg-white rounded-3xl border border-slate-100 shadow-card p-5">
              ...
              <div className="space-y-2">
                {alerts.slice(0, 3).map((a: any) => (
                  <div key={a.id} className={cn("flex items-start gap-2.5 p-3 rounded-xl text-xs",
          ...
          <div className="grid grid-cols-4 gap-3">
            {[
              ...
              { label: "Drug Interactions",  count: interactions.length, icon: AlertOctagon,
                color: interactions.length > 0 ? "text-red-600 bg-red-50 border-red-100" : "text-slate-400 bg-slate-50 border-slate-100" },
              ...
              <div key={label} className={cn("rounded-2xl border p-3 text-center", color)}>
                ...
                <p className="text-[10px] font-medium mt-0.5 opacity-80 leading-tight">{label}</p>
              </div>
            ))}
          </div>
          ...
          {findings.length === 0 && !corrLoading && (
            <div className="bg-white rounded-3xl border border-slate-100 shadow-card p-10 text-center">
              ...
              <p className="text-sm text-slate-400 mb-4">Click "Run Analysis" to generate AI-powered insights</p>
        ...
        <div className="space-y-3">
          <div className="bg-blue-50 border border-blue-100 rounded-2xl p-3.5 flex items-start gap-2.5 mb-4">
            ...
            <p className="text-xs text-slate-700 leading-relaxed">
              <strong>Longitudinal biomarker intelligence</strong> — AI analyses patterns across all your lab reports over time.
            </p>
          </div>
          {corrLoading && [1,2,3].map(i => <div key={i} className="h-24 skeleton rounded-2xl" />)}
      ...
      {activeTab === "progression" && (
        <div className="space-y-3">
          <div className="bg-amber-50 border border-amber-100 rounded-2xl p-3.5 flex items-start gap-2.5 mb-4">
            ...
            <p className="text-xs text-slate-700 leading-relaxed">
              <strong>Disease progression analysis</strong> — tracks recurring conditions and chronic disease trajectories across your full medical history.
            </p>
          </div>
          {!corrLoading && progressionFindings.length === 0 && (
            <div className="text-center py-12 bg-white rounded-3xl border border-slate-100 shadow-card">
              <Activity size={28} className="text-slate-200 mx-auto mb-2" />
              <p className="text-sm text-slate-400">No recurring patterns found. Upload discharge summaries and consultation records.</p>
            </div>
          )}
          {progressionFindings.map((f, i) => <FindingCard key={f.id || i} finding={f} />)}
        </div>
      )}
      ...
      {activeTab === "medicine" && (
        <div className="space-y-3">
          <div className="bg-purple-50 border border-purple-100 rounded-2xl p-3.5 flex items-start gap-2.5 mb-4">
            ...
            <p className="text-xs text-slate-700 leading-relaxed">
              <strong>Medicine intelligence</strong> — correlates your medications with biomarker changes to identify therapeutic effects and side effects.
            </p>
          </div>
          {!corrLoading && medicineFindings.length === 0 && (
            <div className="text-center py-12 bg-white rounded-3xl border border-slate-100 shadow-card">
              <Pill size={28} className="text-slate-200 mx-auto mb-2" />
              <p className="text-sm text-slate-400">No medicine correlations yet. Upload prescriptions and lab reports together.</p>
            </div>
          )}
          {medicineFindings.map((f, i) => <FindingCard key={f.id || i} finding={f} />)}
        </div>
      )}
      ...
      {activeTab === "interactions" && (
        <div className="space-y-3">
          <div className="flex items-center justify-between mb-4">
            <div className="bg-red-50 border border-red-100 rounded-2xl p-3.5 flex items-start gap-2.5 flex-1 mr-3">
              <AlertOctagon size={13} className="text-red-600 shrink-0 mt-0.5" />
              <p className="text-xs text-slate-700 leading-relaxed">
                <strong>Drug interaction checker</strong> — AI analyses all your active medications for potential interactions. Always confirm with your pharmacist or doctor.
              </p>
            </div>
            <button
              onClick={() => intMut.mutate()}
              disabled={intMut.isPending}
              className="flex items-center gap-1.5 px-3 py-2 text-xs font-bold text-slate-600 bg-white border border-slate-200 rounded-xl hover:border-slate-300 shrink-0"
            >
              <RefreshCw size={12} className={intMut.isPending ? "animate-spin" : ""} />
              {intMut.isPending ? "Checking…" : "Check Now"}
            </button>
          </div>

          {intLoading && [1,2].map(i => <div key={i} className="h-24 skeleton rounded-2xl" />)}

          {!intLoading && interactions.length === 0 && (
            <div className="text-center py-12 bg-white rounded-3xl border border-slate-100 shadow-card">
              <CheckCircle2 size={28} className="text-emerald-400 mx-auto mb-2" />
              <p className="font-bold text-emerald-700 mb-1">No interactions detected</p>
              <p className="text-sm text-slate-400 mb-4">
                Either no active medicines on record, or no known interactions found. Click "Check Now" to run a fresh check.
              </p>
              <button onClick={() => intMut.mutate()} disabled={intMut.isPending} className="btn-primary">
                <RefreshCw size={13} /> Run Drug Check
              </button>
            </div>
          )}

          {interactions.filter(i => i.severity === "contraindicated" || i.severity === "major").map(i => (
            <DrugInteractionCard key={i.id} interaction={i} onAcknowledge={id => ackMut.mutate(id)} />
          ))}
          {interactions.filter(i => i.severity === "moderate").map(i => (
            <DrugInteractionCard key={i.id} interaction={i} onAcknowledge={id => ackMut.mutate(id)} />
          ))}
          {interactions.filter(i => i.severity === "minor").map(i => (
            <DrugInteractionCard key={i.id} interaction={i} onAcknowledge={id => ackMut.mutate(id)} />
          ))}
        </div>
      )}
